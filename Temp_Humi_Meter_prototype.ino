...
unsigned long PreRequestTiming  = 0;
...
void loop() {
 ...
 // delay(2000);  // センサーの読み取りを2秒間隔
  if(millis() - SetUpTime - RequestTiming >= 2000){

    RequestTiming = millis()  - SetUpTime;
  ...
  Temp = Temperature;   //表示用に整数化　小数点以下切り捨て
  Humi = Humidity;
...
//表示更新確認用に使用しないところに実行した時間を表示ms（デバッグ用）
  lcd.setCursor(1, 1);
  lcd.print(RequestTiming + SetUpTime);
...
/*デバッグ表示*/
  ...
  Serial.println  (RequestTiming);
  Serial.print    ("実行周期              : ");
  Serial.println  (RequestTiming - PreRequestTiming);

  PreRequestTiming  = RequestTiming;
